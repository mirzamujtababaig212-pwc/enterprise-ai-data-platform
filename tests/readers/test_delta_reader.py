...
def test_empty_delta_reader(spark, temp_dir):
    empty_df = spark.createDataFrame([], schema)
    (empty_df.write.format("delta").mode("overwrite").save(temp_dir))
    reader = DeltaReader(path=temp_dir)
    df = reader.read(spark)
    assert df.count() == 0


def test_delta_reader(spark, temp_dir):
...
def test_large_delta_reader(spark, temp_dir):
    rows = [(i, f"name{i}") for i in range(1000)]
    df = spark.createDataFrame(rows, schema)
    (df.write.format("delta").mode("overwrite").save(temp_dir))
    reader = DeltaReader(path=temp_dir, schema=schema)
    result = reader.read(spark)
