...
def test_empty_parquet_reader(spark, temp_dir):
    empty_df = spark.createDataFrame([], schema)
    empty_df.write.mode("overwrite").parquet(temp_dir)
    reader = ParquetReader(path=temp_dir, schema=schema)
    df = reader.read(spark)
    assert df.count() == 0


def test_parquet_reader(spark, temp_dir):
    df = spark.createDataFrame([(1, "A"), (2, "B")], schema)
    df.write.parquet(temp_dir, mode="overwrite")
    reader = ParquetReader(path=temp_dir, schema=schema)
...
def test_large_parquet_reader(spark, temp_dir):
