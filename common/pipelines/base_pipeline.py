...
class BasePipeline(ABC):
    ...
    def process_batch(
        ...
        for attempt in range(self.config.retries):
                logger.info(
                        "Processing %s Batch %s",
                        self.config.pipeline_name,
                        batch_id
    		)
                try:
                        transformed=self.transformer.transform(batch_df)
                        valid_df, invalid_df = self.validator.validate(
                                transformed
                        )

                        output_df = self.transformer.transform(
                                valid_df
                        )
